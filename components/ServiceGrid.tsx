...
export function ServiceGrid() {
  return (
    <div className="mx-auto mt-12 grid max-w-4xl grid-cols-2 gap-px overflow-hidden rounded-[3px] border border-line-strong bg-line-strong lg:grid-cols-3">
      {services.map((s) => (
        <Link
          key={s.id}
          href={`/services/${s.id}`}
          className="svc-cell group flex flex-col items-center gap-5 bg-ink px-5 py-11 text-center transition-colors duration-500"
        >
          <ServiceIcon
            id={s.id}
